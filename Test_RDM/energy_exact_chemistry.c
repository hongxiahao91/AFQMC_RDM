...
void compute_energy_chemistry_restricted_exact(MKL_Complex16 *wf_up,MKL_Complex16 *wf_down,MKL_Complex16 *weights,MKL_Complex16 *kinetic_original_matrix, MKL_Complex16 *potential_original_matrix,MKL_Complex16 *energy_shifts,MKL_Complex16 *total_potential_energy,MKL_Complex16 *total_kinetic_energy,MKL_Complex16 *total_energy_2,MKL_Complex16 *total_weight,double *weight_rescaling,int_st ist,cns_st cns,int step,MKL_Complex16 *pe,MKL_Complex16 *pe_error,MKL_Complex16 *ke,MKL_Complex16 *ke_error,MKL_Complex16 *te,MKL_Complex16 *te_error,MKL_Complex16 *tw,MKL_Complex16 *normalized_wave_function_up,MKL_Complex16 *normalized_wave_function_down) {
  ...
  */
   ...
   /*Take First Half of Walkers and Cross Them With Second Half of Walkers*/
   for (walker = 0; walker < (int)(ist.n_walkers/2.0) ; walker++ ) {

     //Check That Walker Weights Are Non-Zero
     if ( Cabs(weights[walker]) != 0 ) {
       number_walkers_active++;

       /*Then Cross With Second Half*/
       for (walker2 = (int)(ist.n_walkers/2.0); walker2 < ist.n_walkers; walker2++) {

        // Check Walker 2 Weight Non Zero As Well
        if ( Cabs(weights[walker2]) != 0 ) {   

           //Accumulate Weight in Denominator/
           current_weight = Cmul(RCmul(weight_factor, weights[walker]), RCmul(weight_factor,conjugate(weights[walker2])));
           weight_denominator = Cadd(weight_denominator, current_weight);

           //First Compute the Density Matrix
           compute_density_matrix_chemistry_exact(&wf_up[walker*ist.n_spatial_orbitals_n_up],&wf_down[walker*ist.n_spatial_orbitals_n_down],&wf_up[walker2*ist.n_spatial_orbitals_n_up],&wf_down[walker2*ist.n_spatial_orbitals_n_down],density_matrix_up,density_matrix_down,ist); 
   ...
   for (walker = 0; walker < (int)(ist.n_walkers/2.0) ; walker++ ) {
     ...
     if ( Cabs(weights[walker]) != 0 ) {
      ...
      for (walker2 = (int)(ist.n_walkers/2.0); walker2 < ist.n_walkers; walker2++) {
        ...
        /*Check That Walker Weight Is Non Zero*/
        if ( Cabs(weights[walker2]) != 0 ) {   

          /*Accumulate Weight in Denominator*/
          current_weight = Cmul(RCmul(weight_factor, weights[walker]), RCmul(weight_factor,weights[walker2]));
          weight_denominator = Cadd(weight_denominator, current_weight);
...
void compute_density_matrix_chemistry_exact(MKL_Complex16 *wf_up,MKL_Complex16 *wf_down,MKL_Complex16 *wf_up_2,MKL_Complex16 *wf_down_2,MKL_Complex16 *density_matrix_up,MKL_Complex16 *density_matrix_down,int_st ist) {
  ...
  int i, j;
  MKL_Complex16 *wf_up_2_conjugate, *wf_down_2_conjugate;  
 ...
 /*Declare Overlap Inverse Matrices*/
  ...
  wf_up_2_conjugate=(MKL_Complex16 *)calloc(ist.n_spatial_orbitals_n_up,sizeof(MKL_Complex16)); 
  wf_down_2_conjugate=(MKL_Complex16 *)calloc(ist.n_spatial_orbitals_n_down,sizeof(MKL_Complex16));  
  ...
  /*Conjugate WFs*/
  for (i=0; i<ist.n_spatial_orbitals_n_up; i++) {
    wf_up_2_conjugate[i] = conjugate(wf_up_2[i]); 
  }
  for (i=0; i<ist.n_spatial_orbitals_n_down; i++) {
    wf_down_2_conjugate[i] = conjugate(wf_down_2[i]); 
  } 
 
  /*Run Through the Second Half of All Walkers*/
  compute_overlap_inverse(wf_up_2_conjugate,wf_up,overlap_inverse_up,ist,0);
  compute_overlap_inverse(wf_down_2_conjugate,wf_down,overlap_inverse_down,ist,1); 

  /*Multiply Into Full Wf*/ 
  cblas_zgemm(CblasRowMajor,CblasNoTrans,CblasTrans,ist.n_up,ist.n_spatial_orbitals,ist.n_up,One,overlap_inverse_up,ist.n_up,wf_up_2_conjugate,ist.n_up,Zero,stored_product1_up,ist.n_spatial_orbitals);
  cblas_zgemm(CblasRowMajor,CblasNoTrans,CblasTrans,ist.n_down,ist.n_spatial_orbitals,ist.n_down,One,overlap_inverse_down,ist.n_down,wf_down_2_conjugate,ist.n_down,Zero,stored_product1_down,ist.n_spatial_orbitals);
...
free(wf_up_2_conjugate); free(wf_down_2_conjugate); 
